...
{
	[TestFixture]
	public class When_validating_a_property_as_null
	{
		...
		public void ensure_not_null_returns_false()
		{
			...
			Assert.AreEqual("value must be null", violations[0].ErrorMessage);
		...
		private class TestClassValidator : Validator<TestClass>
		{
			...
			{
				Ensure(x => x.value.IsNull());
				Ensure(x => x.value.IsNull("test message"));
			}
		}
	}
}